...
function processPattern():void {
  parsedPattern.value = dynamicListParser(patternInputValue.value.replace (/\n/g,'-n'))
...
<template>
  ...
  <main>
    ...
    <br>
    <h2>## parsed list</h2>
    <textarea class="plain-textarea" ref="outputTextareaRef" v-model="plainOutputValue" readonly></textarea>
  </main>
</template>
...
function dynamicListParser(pattern: string): ParseResult {
  // 1 character each
  const flagToken = '-'
  const wordToken = 'w'
  const digitToken = 'd'
  ...
  const starterTokens = groupTokens.map((g) => { return '\\'+g.start }).join('')
  const terminatorTokens = groupTokens.map((g) => { return '\\'+g.end }).join('')

  // TODO: make this regexp smarter
  const tokensRx = new RegExp(`${wordToken}|${digitToken}|${boolToken}|${repToken}|${lnToken}|\\${orToken}`, 'g')
  ...
  for (let pI = 0; pI < pattern.length; pI++) {
    ...
    // match terminators
    if (!c.escaped && (terminatorsRx.test(c.raw) || c.raw == logicGroupTokens.end)) {
      ...
      if (
        ...
        return createResult(true, true, `Incorrectly closed groups! (char:${pI})`)
      }
    }
  ...
  for (let sI = 0; sI<segments.length; sI++) {
    ...
    if (segments[sI].type == blockType.repetitive && sI < segments.length - 1 && !(terminatorsRx.test(segments[sI+1].raw) || segments[sI+1].raw == logicGroupTokens.end)) {
      return createResult(true, true, `Repeat token set incorrectly! (segm:${sI})`)
    }
  }
  ...
  const typeWithinGroup = (segments: Segment[], segmentIndex: number, type: blockType): boolean => {
    const pairIndex = segments.findIndex((s, i) => s.segmentId == segments[segmentIndex].segmentId && i > segmentIndex)
    if (pairIndex < 0) return false
    const conincidenceIndex = segments.findIndex((s, i) => s.type == type && i < pairIndex) 
  ...
  for (let sI = 0; sI < segments.length; sI++) {
    ...
    if (curS.type === blockType.logicGroup) {
      if (curS.raw === '<') {
        const newBlock = createBlock('', '', false, typeWithinGroup(segments, sI, blockType.or), false, []);
        ...
        while (blockStack.length > 1 && !blockStack[blockStack.length - 1].options) {
          blockStack.pop();
        }
      ...
      while (blockStack.length > 2 && !blockStack[blockStack.length - 2].options) {
        blockStack.pop();
      }
      currentOption = createBlock('', '', false, false, true, []);
      ...
      currentBlock.children.push(createSegment(curS.raw, curS.type, curS.segmentId, curS.escaped));
    }
  }
